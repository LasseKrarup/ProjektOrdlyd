        ...
        <Layout bg="#4C4F57" header={false}>
          ...
          <div className="w-screen overflow-hidden absolute bottom-0">
            <div style={{width: "150%"}} className="relative -left-1/4">
              <motion.img initial={{opacity: 0}} animate={{opacity: 1,}} exit={{opacity: 0, transition: {delay: 0.5}}} className="w-full relative" src={OrdlydBg} alt="Projekt Ordlyd logo" ></motion.img>
              <motion.img transition={{...transition, duration: 1, delay: 0.5}} className="w-40 absolute bottom-0 left-1/2 transform -translate-x-1/2" src={OrdlydSingle} alt="Projekt Ordlyd logo" ></motion.img>
            </div>
          </div>
        </Layout>
  );
